...
export default function AllVideos({ allVideos }: AllVideosProps) {
    const [AllvideoData] = useState<VideoType[] | null>(allVideos)
    const navigate = useNavigate()


    return (
        <div className="flex flex-col gap-[25px] px-3 mt-[30px] pb-24">
            {AllvideoData && AllvideoData.map((video) => (
                <div onClick={() => navigate(`/player/${video.video_youtube_id}`)} key={video._id}>
                    <img src={video.video_img_url} alt="Video Thumnail" className="w-full h-[240px] rounded-[5px]" />
