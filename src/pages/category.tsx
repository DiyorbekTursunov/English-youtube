...
export default function Category() {
    ...
    function groupByVideoType(videos: VideoType[]): Record<string, VideoType[]> {
                ...
                loading ? (
                    ...
                    <div className="w-full h-full bg-[#F0EEED]">
                        ...
                        <div className='max-w-[500px] mx-auto pb-20 pt-5'>
                            {searchIsOpen || categoryIsOpen ? <div className="fixed w-full h-screen bg-[#000000] z-30 opacity-40" onClick={() => { setsearchIsOpen(false), setcategoryIsOpen(false) }}></div> : null}

                            <div className="max-w-[500px] mx-auto  w-full bg-white flex justify-between shadow-md py-4 top-0 px-5 fixed z-50">
                                <button onClick={() => { setsearchIsOpen(!searchIsOpen), setcategoryIsOpen(categoryIsOpen && !categoryIsOpen) }}>
                                ...
                                <button onClick={() => { setsearchIsOpen(searchIsOpen && !searchIsOpen), setcategoryIsOpen(!categoryIsOpen) }}>
                                    <svg width={20} viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg" fill="none"><g id="SVGRepo_bgCarrier" strokeWidth="0"></g><g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g><g id="SVGRepo_iconCarrier"> <path fill="#000000" fillRule="evenodd" d="M19 4a1 1 0 01-1 1H2a1 1 0 010-2h16a1 1 0 011 1zm0 6a1 1 0 01-1 1H2a1 1 0 110-2h16a1 1 0 011 1zm-1 7a1 1 0 100-2H2a1 1 0 100 2h16z"></path> </g></svg>
                                </button>
                            </div>
                            {<div className={`${categoryIsOpen ? "top-[50px]" : "top-[-900%]"} max-w-[500px] mx-auto w-full transition-all duration-400 grid grid-cols-2 gap-x-5 py-5 shadow-md px-3 gap-y-3 fixed z-40 bg-[#F0EEED]`}>
                                {[...new Set(allVideos.map(video => video.video_type))].map((videoType, index) =>
                                    <button
                                        onClick={() => { setcategory(index), setcategoryIsOpen(false) }}
                                        key={index}
                            ...
                            {<div className={`${searchIsOpen ? "top-[50px]" : "top-[-900%]"} max-w-[500px] mx-auto w-full transition-all duration-400 py-5 shadow-md px-3 gap-y-3 fixed z-40 bg-[#F0EEED]`}>
                                <input type="text" className="py-2 pl-2 placeholder:text-sm placeholder:font-medium w-full" placeholder="Qidirish" />
                            </div>}

                            {Object.keys(groupedVideos).map((key, index) => (
                                category === index && (
